...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class DepthNormalOnlyPass : ScriptableRenderPass
    {
        ...
        {
            // Forces float format for "hijacking" normals buffer
            if (SystemInfo.IsFormatSupported(GraphicsFormat.R16G16B16A16_SFloat, GraphicsFormatUsage.Render))
                return GraphicsFormat.R16G16B16A16_SFloat; // Preferred format for 3dxel hijacking
            else if (SystemInfo.IsFormatSupported(GraphicsFormat.R32G32B32A32_SFloat, GraphicsFormatUsage.Render))
                return GraphicsFormat.R32G32B32A32_SFloat; // fallback
            else
                return GraphicsFormat.R8G8B8A8_SNorm; // fallback
        }
